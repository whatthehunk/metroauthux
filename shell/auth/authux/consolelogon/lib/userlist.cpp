...
HRESULT UserList::AddTileFromData(const Microsoft::WRL::ComPtr<LCPD::ICredential>& credential, const Microsoft::WRL::ComPtr<LCPD::IUser> user, HSTRING userName)
{
	...
	// Metro deselected-state tile (build 7878 recreation).
	// If the UIFILE contains a LogonUserTileList selector (the horizontal
	// Metro tile row), create a LogonUsertileElement for it.
	DirectUI::Selector* pMetroList = (DirectUI::Selector*)
		FindDescendent(DirectUI::StrToID(L"LogonUserTileList"));
	if (pMetroList)
	{
		CDUILogonUsertileElement* pMetroTile = nullptr;
		if (SUCCEEDED(m_xmlParser->CreateElement(
			L"LogonUserTileTemplate", nullptr,
			pMetroList, nullptr,
			(DirectUI::Element**)&pMetroTile)) && pMetroTile)
		{
			pMetroTile->m_owningUserList = this;
			if (SUCCEEDED(pMetroTile->Configure(user, credential, m_xmlParser)))
			{
				pMetroTile->SetVisible(true);
				pMetroList->Add(pMetroTile);
			}
			else
			{
				pMetroTile->Destroy(true);
			}
		}
	}
